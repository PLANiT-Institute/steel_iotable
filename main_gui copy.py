...
def show_io_analysis():
    ...
    if analyze_button or st.session_state.get('auto_analyze', False):
        ...
        # Calculate all coefficient effects using the analyzer's method
        ...
        with tabs[0]:
            ...
            if all_summary_data:
                ...
                try:
                    ...
                    # Fallback to combined CSV using the analyzer's method
...
def show_hydrogen_analysis():
    ...
    if analyze_button or st.session_state.get('auto_analyze_hydrogen', False):
        ...
        with st.spinner("Calculating all hydrogen coefficient effects..."):
            all_results, coefficient_types, coeff_names = h_analyzer.calculate_all_effects(
                selected_scenario,
                demand_change,
                quiet=True  # Suppress output for GUI
            )
        ...
        with tabs[0]:
            ...
            if all_summary_data:
                ...
                try:
                    ...
                    st.info("Using CSV export instead.")
                    # Fallback to combined CSV using the analyzer's method
                    combined_data = h_analyzer.create_combined_data(all_results, coefficient_types, coeff_names)

                    # Add scenario column
                    for row in combined_data:
                        row['scenario'] = selected_scenario
        ...
        for i, coeff_type in enumerate(coefficient_types):
            with tabs[i + 1]:  # +1 because summary tab is at index 0
                ...
                if results['impacts']:
                    ...
                    # Download button
                    csv_data = df[['sector_code', 'sector_name', 'impact']].to_csv(index=False, encoding='utf-8-sig')
                    st.download_button(
                        label=f"📥 Download {coeff_names[coeff_type]} Results",
                        data=csv_data,
