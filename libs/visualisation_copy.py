...
class Visualization:
    ...
    def load_all_results(self):
        ...
        """
        # 1. Load individual scenario results (directly in output dir)
        self.results = self._load_scenario_result_csvs(self.output_dir)

        # 2. Load integrated scenario results (directly in output dir)
        self.integrated_results = self._load_integrated_scenarios_csvs(self.output_dir)

        # 3. Load code_h aggregated results (if subdirectory exists)
    ...
    def _load_scenario_result_csvs(self, directory: str) -> Dict[str, Dict[str, pd.DataFrame]]:
        ...
        Expected filename format: scenario_{effect_type}_{sector}_{input_table}.csv
        e.g., scenario_indirect_prod_1610_iotable_2020.csv
        """
        results = {}
        ...
        known_scenarios = ['1610', '4506', 'H2S', 'H2T']
        #known_scenarios = ['1610']

        for fname in os.listdir(directory):
            
            if fname.endswith(".csv") and fname.startswith("scenario_"):
                # Remove 'scenario_' prefix and '.csv' suffix
                name_part = fname[9:-4]  # Remove 'scenario_' (9 chars) and '.csv' (4 chars)

                # Find which scenario this is
                scenario_id = None
                for scenario in known_scenarios:
                    if f"_{scenario}_" in name_part:
                        # Split at the scenario to get effect_type
                        parts = name_part.split(f"_{scenario}_")
                        if len(parts) == 2:
                            effect_type = parts[0]
                            scenario_id = scenario
                            break

                if scenario_id and effect_type:
    ...
    def _load_integrated_scenarios_csvs(self, directory: str) -> Dict[str, Dict[str, pd.DataFrame]]:
        """Load integrated (1610+4506, H2S+H2T) results.

        Expected filename format: scenario_{effect_type}_{scenario_name}_{input_table}.csv
        Note: Each CSV contains ALL years as columns, not one CSV per year.
        Returns: Dict[scenario_name][effect_type] = DataFrame with year columns
        """
        results = {}
        if not os.path.isdir(directory):
        ...
        known_integrated = ['1610+4506', 'H2S+H2T']
        ...
        for fname in os.listdir(directory):
            if fname.endswith(".csv") and fname.startswith("scenario_"):
                ...
                # Find which integrated scenario this is
                scenario_name = None
                for scenario in known_integrated:
                    if f"_{scenario}_" in name_part:
                        # Split at the scenario to get effect_type
                        parts = name_part.split(f"_{scenario}_")
                        if len(parts) == 2:
                ...
                if scenario_name and effect_type:
                    df = pd.read_csv(os.path.join(directory, fname))
                    # Store as: results[scenario][effect_type] = df (df has year columns)
                    results.setdefault(scenario_name, {})[effect_type] = df

        #print(results['1610_4506']['indirect_import']['2026'].sum())
        #exit()
        print(f"  ✅ Loaded integrated scenario CSVs: {len(results)} scenarios")
        return results
    ...
    def create_io_yearly_trends(self, effect_type: str, scenarios: List[str] = None,
        ...
        effect_info = {
            ...
            'indirect_import': {'label': 'IndirectImport', 'unit': 'Billion Won'},
        ...
        for scenario in scenarios:
            if scenario == '1610+4506':
                # Load integrated scenario CSV - format: Sector_Code | Sector_Name | 2026 | 2027 | ...
                if scenario in self.integrated_results and effect_type in self.integrated_results[scenario]:
                    df = self.integrated_results[scenario][effect_type]
                    # Year columns are numeric strings like '2026', '2027', etc.
                    year_cols = [col for col in df.columns if col.isdigit()]
                    all_years = sorted([int(col) for col in year_cols])

                    # Filter to year range 2026-2050
                    years = [y for y in all_years if 2026 <= y <= 2050]

                    values = []
                    for year in years:
                        # Sum all sectors for this year
                        total = df[str(year)].sum()
                        # Convert to billion won for monetary values
                        if effect_type in ['indirect_prod', 'indirect_import', 'value_added', 'valueaddedcoeff']:
                            total = total / 1000
                        values.append(total)
                ...
                # Individual scenarios - format: Sector_Code | Sector_Name | 2020 | 2025 | ...
                if effect_type in self.results and scenario in self.results[effect_type]:
                    df = self.results[effect_type][scenario]
                    # Get year columns (those that are numeric)
                    year_cols = [col for col in df.columns if col.isdigit()]
                    years = sorted([int(col) for col in year_cols if col.isdigit()])
                    ...
                    # Filter to year range 2026-2050
                    filtered = [(y, v) for y, v in zip(years, values) if 2026 <= y <= 2050]
                    if filtered:
            ...
            if scenario == 'H2S_H2T':
                ...
                if scenario in self.integrated_results and effect_type in self.integrated_results[scenario]:
                    ...
                    for year in years:
                        # Sum all sectors for this year
                        total = df[str(year)].sum()
                        # Convert to billion won for monetary values (not for directemploycoeff)
                        if effect_type != 'directemploycoeff':
                            total = total / 1000
                        values.append(total)
                ...
                # Individual scenarios - format: Sector_Code | Sector_Name | 2026 | 2027 | ...
                if effect_type in self.results and scenario in self.results[effect_type]:
                    df = self.results[effect_type][scenario]
                    # Get year columns (those that are numeric)
                    year_cols = [col for col in df.columns if col.isdigit()]
    ...
    def create_all_trends(self, output_dir: str = 'output/plotly_charts', save_html: bool = True):
        ...
        for effect in io_effects:
            ...
            # Explicitly include integrated scenario 1610+4506
            fig = self.create_io_yearly_trends(effect, scenarios=['1610', '4506', '1610+4506'], show_fig=False)
            if save_html:
        ...
        for effect in h2_effects:
            ...
            # Explicitly include integrated scenario H2S+H2T
            fig = self.create_hydrogen_yearly_trends(effect, scenarios=['H2S', 'H2T', 'H2S_H2T'], show_fig=False)
            if save_html:
...
if __name__ == "__main__":
    ...
    #print(viz._load_scenario_result_csvs('output'))
